  ...
  let timers = clockString(_timers)
   ...
   try {
     ...
     let mentionedJid = [m.sender]
      ...
      conn.send2Button(m.chat, pler, `Games wabot`, `Ya`, `+dya`, `No`, `+dno`, m, false, { contextInfo: { mentionedJid : user}})
     ...
     if(kenal) throw 'Anda siapa?\nkok ikut kut mau duel'
     user.lastduel = new Date * 1
     if (Aku > Kamu) {
       user.money -= 100000
       enemy.money += 100000
       delete conn.duel[m.sender]
       conn.reply(m.chat, `@${who.split("@")[0]} Menang GeludðŸ¤¼\n*Hadiah:*\n100000 Money buat beli gorengan`.trim(), m)
     } else if (Aku < Kamu) {
       user.money += 100000
       enemy.money -= 100000
       delete conn.duel[m.sender]
       conn.reply(m.chat, `@${who.split("@")[0]} Kalah GeludðŸ¤¼\n*Hadiah:*\n 100000 money Mayan buat beli Limit`.trim(), m)
     } else {
       user.money += 10000
       enemy.money += 10000
       delete conn.duel[m.sender]
       conn.reply(m.chat, `@${who.split("@")[0]}\n *Seri*\nmasing masing 10000 Money`.trim(), m)
     }
   }
   if (/dno/.test(command)) {
   let kenal = !who.includes(m.sender)
   if(kenal) return conn.send2Button(m.chat, `Anda siapa?\nkok ikut kut mau duel`, `Sesion`, `NO`, `.dno`, m)
    //if (!who) return m.reply('tag yg ingin di ajak duel!')
    conn.reply( m.chat, `@${who.split("@")[0]} Membatalkan Ajakan Duel`, m)
    delete conn.duel[m.sender]
   }
 } catch (e) {
   //return conn.send2Button( m.chat, `Sepertinya ada bug`, `laporkan ke owner`, `Donasi`, `/request duel ${e.stack}`, m)
   return m.reply(`${e}`)
 }
}
